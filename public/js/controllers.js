function introController($scope, $cookies, $location) {
    $scope.showIntro = true
    if (!$cookies.getAll()["showIntro"] && $location.path() == "/") {
        $scope.showIntro = true
        $cookies.put("showIntro", "Shown", { expires: new Date("January 01, 2020 00:00:00") })
    } else {
...
introController.$inject = ["$scope", "$cookies", "$location"]
...
function navigationController($scope, $location, starService) {
    ...
    $scope.$on('$routeChangeSuccess', function(event, current, previous) {
        ...
        if ($location.path() == "/createstar") {
            $scope.createstar = true
        }
        if ($location.path() == "/") {
            $scope.home = true
        }
...
function chooseStarController($scope, starService, $location, selectedStarService, $uibModal, $anchorScroll) {
    ...
    $scope.$on("$routeChangeStart", function(eve, next, current) {
        ...
        if (nextPath != "/createstar/name" && self.selectedStar) {
...
function nameStarController($scope, selectedStarService, $uibModal, $location, facebookService, $timeout, vcRecaptchaService) {
    ...
    if (!selectedStarService.selectedStar.selected) {
        return $location.path("/createstar/choose")
    } else {
        ...
        $scope.createStar = function() {
            ...
            if ($scope.forms.starForm.starName.$error.minlength) {
                $scope.errors.push("Star Name is to be minimum 3 characters")
                $scope.required.name = true
            }

            if ($scope.forms.starForm.starName.$error.maxlength) {
                $scope.errors.push("Star Name can be maximum 15 of characters")
                $scope.required.name = true
            }

            if (!selectedStarService.selectedStar.starImage) {
                $scope.errors.push("A Star Picture is required")
            }
            ...
            if (!selectedStarService.selectedStar.user.pictureUrl) {
                ...
                $scope.required.yourPicture = true
            }
            ...
            if (!selectedStarService.selectedStar.user.email) {
            ...
            if ((selectedStarService.selectedStar.user.email != $scope.confirm.email)  && $scope.fbLoginStatus) {
        ...
        this.setFbData = function(fbData, Userdata) {
            ...
            if (Userdata.email) {
                ...
                $scope.fbEmailId = true
            } else {
                $scope.fbEmailId = false
            }
            user.fbData = fbData
                ...
                .then(function(loginResponse) {
                    ...
                    facebookService.fbMe()
                        .then(function(userImageResponse) {
                            ...
                            $scope.confirm.email = selectedStarService.selectedStar.user.email
    ...
    selectedStarService.createStar(res.payment)
        .then(function(response) {
            if (response.redirect) {
                ...
                $scope.username = response.userName.split(" ")[0]
                $scope.name = response.starName.toUpperCase()
            }
        }, function(err) {
            $scope.msg = err;
            $scope.resultWaiting = false;
...
function imageSelectorController($scope, selectedStarService, Upload, $uibModalInstance, $timeout) {
    ...
    $scope.setFile = function(element) {
        ...
        loadImage(
            element.files[0],
            function(canvas) {
                $scope.image_source = canvas.toDataURL("image/png");
                $scope.$apply()
            }, { maxWidth: 1200, maxHeight: 1200, orientation: true } // Options
        );
    }
...
function starPageController($scope, $routeParams, starPageService, response, Lightbox, $location, facebookService, $uibModal) {
    ...
    var self = this
    if (!response.error) {
        // Show popup for Payment verification if star is disabled
...
function paymentVerificationModalController($scope, res, paymentService, miscService, facebookService, $window, $uibModalInstance) {
    ...
    paymentService.getPaymentConfirmation(res.starData.name, res.payment_id, res.payment_request_id, res.paymentId, res.PayerID, res.token)
        .then(function(response) {
            ...
            $scope.username = res.starData.user.name.split(" ")[0]
            $scope.name = res.starData.name.toUpperCase()
        }, function(err) {
            $scope.msg = err
            $scope.resultWaiting = false
            $scope.buttonMsg == "OK"
            $scope.errHeading = true
        });
